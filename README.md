...
For the current World ID 4.x flow, create an app, RP, and action in the [World Developer Portal](https://developer.world.org), then use IDKit 4.x with the simulator's staging environment or production World App. Set `WORLD_ID_APP_ID`, `WORLD_ID_RP_ID`, `WORLD_ID_SIGNING_KEY`, `WORLD_ID_ENVIRONMENT`, and `WORLD_ID_ALLOWED_ACTIONS`, then start `npm --workspace @verity/world-id-service start`. Open the service root at `/` to obtain a proof: enter the configured action and exact signal, approve it in World App or the simulator, and copy the complete IDKit result from the page. The page calls `POST /rp-signature` and verifies the result through `POST /verify-proof` before displaying it. The backend gives the client a signed request context and forwards the complete IDKit response unchanged to `WORLD_ID_VERIFY_URL`. Set that URL to `https://developer.world.org/api/v4/verify/<rp_id>`. The proof's `responses[].signal_hash` must be generated from the exact signal supplied to `register:provider` or `dispute:demo`; the verifier stores the returned nullifier as the durable human root. See the [IDKit integration guide](https://docs.world.org/world-id/idkit/integrate) for the proof request and RP context.
